...
#키워드가 있는 영화의 id가져오기
def get_movie_ids_by_keyword(keyword_name):
     # 키워드 이름을 통해 keyword_id 가져오기
    keyword_id_result = session.query(models.Keyword.keyword_id).filter_by(keyword_name=keyword_name).first()
    ...
    movie_ids = session.query(models.KeywordMovie.movie_id).filter_by(keyword_id=keyword_id).all()
